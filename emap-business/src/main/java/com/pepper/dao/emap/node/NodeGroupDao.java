...
public interface NodeGroupDao extends BaseDao<NodeGroup> {
    ...
    @Query("select t1 from NodeGroup t1 where t1.code in (select t2.code from NodeGroup t2 where t2.nodeId = :nodeId ) and t1.nodeId <> :nodeId1")
    public List<NodeGroup> findAllByNodeId(String nodeId,String nodeId1);
}
