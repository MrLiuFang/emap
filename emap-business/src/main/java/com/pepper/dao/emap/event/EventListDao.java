...
public interface EventListDao extends BaseDao<EventList>, EventListDaoEx {
	 ...
	 */
	@Modifying
	@Query(" update EventList set operator = ?1 where  operator=?2 and status <> 'P' and warningLevel >-1 ")
	...
	@Query("select t4.name as name,  count(t1.id) as count from EventList t1 join Node t2 on t1.sourceCode = t2.sourceCode " +
			" join NodeType t3 on t2.nodeTypeId = t3.id join NodeClassify t4 on t3.nodeClassifyId = t4.id  where   t1.createDate>= ?1 and t1.createDate<= ?2 and t1.isConsole is null and  t1.warningLevel >-1 group by t4.name ")
	public List<Map<String,Object>> currentMonthCount(Date startDate,Date endDate);

	@Query("select '人工申報' as name,  count(t1.id) as count from EventList t1  where   t1.createDate>= ?1 and t1.createDate<= ?2 and t1.isConsole is true and  t1.warningLevel >-1 ")
	public List<Map<String,Object>> currentMonthIsConsoleCount(Date startDate,Date endDate);

	@Query("select 'others' as name,  count(t1.id) as count from EventList t1  where t1. sourceCode not in (select t2.sourceCode from Node t2 )  and   t1.createDate>= ?1 and t1.createDate<= ?2 and t1.isConsole is null and  t1.warningLevel >-1 ")
